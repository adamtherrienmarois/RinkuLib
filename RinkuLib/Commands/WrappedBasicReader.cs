...
internal sealed class WrappedBasicReader : DbDataReader, IDbColumnSchemaGenerator {
    ...
    private static Func<DataRow, DataColumnCollection, DbColumn> GetPrivateDataRowCtor() {
        ...
        var castExp = Expression.Convert(newExp, typeof(DbColumn));
        var lambda = Expression.Lambda<Func<DataRow, DataColumnCollection, DbColumn>>(
            castExp, rowParam, colsParam);
    ...
    public ReadOnlyCollection<DbColumn> GetColumnSchema() {
        ...
        if (schemaTable != null) {
            ...
            foreach (DataRow row in schemaTable.Rows) {
                ...
                columnSchema.Add(Create(row, schemaTableColumns));
            }
        }
